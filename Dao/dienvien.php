...
function get_img($id){
    $sql = "SELECT img_dv FROM dienvien  WHERE id_dienvien=?";
    $getimg=pdo_query_one($sql,$id);
    return $getimg['img'];
}
function dienvien_checkName($name){
    return pdo_query_one("SELECT * from dienvien where ten_dv=?",$name);
}
function dienvien_insert($name,$img){
    $sql = "INSERT INTO dienvien(ten_dv,img_dv) VALUES (?,?)";
    pdo_execute($sql, $name,$img);
}
function dienvien_update($id, $name,$img){
    $sql = "UPDATE dienvien SET  ten_dv=?,img_dv=? WHERE id_dienvien=?";
